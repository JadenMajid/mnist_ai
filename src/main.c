...
int main(int argc, char **argv) {
  ...
  int opt;
  while ((opt = getopt_long(argc, argv, "t:e:l:b:sf:h", long_options, NULL)) !=
         -1) {
    switch (opt) {
    case 't':
      topology = parse_topology(optarg);
      break;
    case 'e':
      epochs = atoi(optarg);
      break;
  ...
  if (topology) {
    ...
    for (int i = 0; i < topology->rows; i++) {
      printf("%d%s", (int)topology->data[i],
             (i == topology->rows - 1) ? "" : " -> ");
    }
    printf("\n");
  }
  ...
  // Mat *X_train =
  // read_mnist_images("archive/augmented_mnist/train-images.idx3-ubyte"); Mat
  // *Y_train =
  // read_mnist_labels("archive/augmented_mnist/train-labels.idx1-ubyte"); Mat
  // *X_test =
  // read_mnist_images("archive/augmented_mnist/t10k-images.idx3-ubyte"); Mat
  // *Y_test_raw =
  // read_mnist_labels("archive/augmented_mnist/t10k-labels.idx1-ubyte");
